...
def load_pcd_file(pcd_path):
    ...
    return np.asarray(lidar.points)
    ...
    for camera_name in camera_params.keys():
        ...
        if camera_identifier in sensor_names:
            data_dict[camera_identifier] = Camera(
                image=load_camera_image(camera_dict[camera_name]),
                sensor2lidar_rotation=camera_params[camera_name]["sensor2lidar_rotation"],
                sensor2lidar_translation=camera_params[camera_name]["sensor2lidar_translation"],
                intrinsics=camera_params[camera_name]["intrinsics"],
                distortion=camera_params[camera_name]["distortion"],
            )
        else:
